      ...
      <div className='flex flex-col'>
        {menuList.map((item) => (
          <div
            ...
            className={`flex gap-2 items-center p-4 px-6 text-gray-500 hover:bg-gray-100 cursor-pointer ${activeIndex === item.id - 1 ? 'bg-purple-50 text-purple-800' : null}`}
            onClick={() => { setActiveIndex(item.id - 1) }} // Adjusted index to match item.id
          >
            <item.icon />
            <h2>{item.name}</h2>
          </div>
